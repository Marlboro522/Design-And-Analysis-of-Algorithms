...
fn f(x:i32) -> i32{
    (0.5 * x as f64 * (x as f64 - 1.0) + 10.0) as i32
}
...
fn plot(range: RangeInclusive<i32>,filename: &str, title: &str, step: usize) {
    let x_vals: Vec<i32> = range.step_by(step).collect();
    let f_vals: Vec<i32> = x_vals.iter().map(|&n| f(n)).collect(); //output of the functionn f(x)
    let g_vals: Vec<i32> = x_vals.iter().map(|&n| g(n)).collect(); //output of the fucntion g(x)
...
fn main(){
    ...
    plot(1..=100000, "plot_1_to_1000000.html", "Functions f(n) and g(n) for n ranging from 1 to 10^6",1000);
}